    ...
    const unsubscribe = listenForActiveQuiz(studentClass, (quiz) => {
      ...
      if (quiz && quiz.id && quiz.status === "active") {
        ...
        console.log("🎯 Decision logic:", {
          ...
          pathname: location.pathname,
          willRedirect:
            isNewQuiz && justStarted && !alreadyHandled && onDashboard,
          willShowBanner:
            !alreadyHandled && (!isNewQuiz || !justStarted || !onDashboard),
        });
        ...
        // - Or if already redirected once
          ...
          console.log(
            "📢 SHOW BANNER - Late joiner or not on dashboard (timeSinceStart: " +
              Math.floor(safeTimeSinceStart / 1000) +
              "s, page: " +
              location.pathname +
              ")",
          );
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      const remaining = Math.floor(
        (activeQuiz.endTime.getTime() - Date.now()) / 1000,
      );
      if (remaining <= 0) {
        setShowBanner(false);
        setActiveQuiz(null);
        ...
        <div>
          ...
          <p className="text-sm opacity-90">
            ...
            <span className="font-bold">
              {formatTime(activeQuiz.remainingSeconds)}
            </span>
          </p>
        </div>
        <button
